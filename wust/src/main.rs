use std::{borrow::BorrowMut, fs, io::{BufRead, BufReader, Read, Write}, net::{TcpListener, TcpStream}, ops::Deref, str::FromStr, usize};

fn handle_request_thread_function(id: usize, queue: std::sync::Arc<wust::Queue<&str>>)
{
	loop {
		queue.as_ref().get();
	}
...
fn server_loop(listener: TcpListener, queue: std::sync::Arc<wust::Queue<&str>>)
{
...
fn main() {
	...
	let thread_number: usize = 10;
	let mut queue = std::sync::Arc::new(wust::Queue::new(5));

	for id in 0..thread_number {
		let new_queue = std::sync::Arc::clone(&queue);
		std::thread::spawn(move || { handle_request_thread_function(id,new_queue); });
	}  
